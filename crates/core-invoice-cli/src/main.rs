...
#[command(
    name = "core-invoice",
    version,
    about = "EN 16931 + Peppol PINT, offline. Fatal ids comparable to pinned ConnectingEurope / PINT-MY via task svrl. Peppol BIS pin is .sch only. Not IRBM Valid. CII is a D16B subset for EN/Peppol; PINT-MY is UBL-only."
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
