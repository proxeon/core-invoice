...
Offline binary `core-invoice`. No network. Fatal ids comparable to pinned ConnectingEurope / PINT-MY via `task svrl`. Peppol BIS pin is `.sch` only. **Not IRBM Valid.** `inspect` prints fields, not a valid/invalid verdict.
